...
if (shoot_fireball and global.points > 0 and fireball_timer <= 0) {
	global.points -= 1
	fireball_timer = 20
	new_fireball = instance_create_layer(x, y, "Instances", oFireball)
...
if (abs(vel_x) < 0.1) {
	if (vel_y > 0) {
		sprite_index = asset_get_index("mc_walkdown")
	}
	if (vel_y < 0) { 
		sprite_index = asset_get_index("mc_walkup")
	}
}
else {
...
length = sqrt(power(vel_x, 2) + power(vel_y, 2))

if (length > 0) {
	var new_pos_x = x + vel_x / length * (1 + boost)
	var new_pos_y = y + vel_y / length * (1 + boost)
	var collision = false
